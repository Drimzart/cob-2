...
export default function Galeria() {
  ...
  const anterior = () => {
    setIndice((prev) => (prev - 1 + casos.length) % casos.length);
  };

  const proximo = () => {
    setIndice((prev) => (prev + 1) % casos.length);
  };

  return (
    <section id="galeria" className="relative bg-gradient-to-b from-white to-slate-100 py-20 px-6 overflow-hidden">
        ...
        <div className="grid md:grid-cols-3 gap-10 items-start">
              ...
              {["antes", "depois", "antes-boca", "depois-boca"].map((tipo, i) => (
                ...
                >
                  ...
                  <img
                    ...
                    alt={`${tipo} ${indice + 1}`}
                    className={`rounded-xl shadow-md object-cover mx-auto w-full ${
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-4 mb-6">
              <button
                onClick={anterior}
                className="p-2 bg-[#053f79]/10 hover:bg-[#053f79]/20 rounded-full"
              >
                <ChevronLeft className="text-[#053f79]" />
              </button>
              <motion.img
                key={indice}
              ...
              <button
                ...
                className="p-2 bg-[#053f79]/10 hover:bg-[#053f79]/20 rounded-full"
              >
                <ChevronRight className="text-[#053f79]" />
              </button>
            </div>
              ...
              {[-2, -1, 1, 2].map((offset) => {
                ...
                return (
                  <motion.img
                    ...
                    alt={`Miniatura ${pos + 1}`}
                    className="w-12 h-16 object-cover rounded border border-gray-200 shadow-sm cursor-pointer opacity-70 hover:opacity-100"
                  />
                );
              })}
            </div>
